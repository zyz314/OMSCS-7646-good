...
def test_code(seed=1489683273):  		  	   		 	   			  		 			     			  	 
    ...
    return lr_check,dt_check
  		  	   		 	   			  		 			     			  	 
if __name__ == "__main__":  		  	   		 	   			  		 			     			  	 
    np.random.seed(42)
    lr_checks = np.array(list())
    dt_checks = np.array(list())
    for seed in set(np.random.randint(low = 0, high=1_000_000, size=2000)):
        lr_check,dt_check = test_code(seed)  		  	   		 	   			  		 			     			  	 
        lr_checks = np.append(lr_checks,lr_check)
        dt_checks = np.append(dt_checks,dt_check)

    print(f"Success Rate: lr - {(lr_checks.sum()/len(lr_checks)):.2%} || dt - {(dt_checks.sum()/len(dt_checks)):.2%}")
    
    np.savetxt("lr_checks.csv",lr_checks,delimiter=",")
    np.savetxt("dt_checks.csv",dt_checks,delimiter=",")